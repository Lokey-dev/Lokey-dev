...
import { liveStats, organizers, scrims, tournaments } from "@/lib/mock-data";
    ...
    <main className="relative overflow-hidden">
      ...
      <section className="container-arena grid min-h-[calc(100vh-5rem)] items-center gap-12 py-16 lg:grid-cols-[1.05fr_.95fr]">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .7 }}>
          <Badge className="border-arena-cyan/40 bg-arena-cyan/10 text-arena-cyan"><Sparkles size={14} /> Funded-startup esports platform mockup</Badge>
        ...
        <motion.div initial={{ opacity: 0, scale: .94 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: .8 }} className="relative">
          <div className="absolute inset-8 rounded-full bg-arena-cyan/20 blur-3xl" />
          <Card className="relative overflow-hidden p-5 animate-float"><div className="rounded-3xl border border-white/10 bg-[linear-gradient(135deg,rgba(168,85,247,.28),rgba(34,211,238,.12))] p-5"><div className="flex items-center justify-between"><Badge>LIVE CONTROL ROOM</Badge><Gamepad2 className="text-arena-cyan" /></div><div className="mt-12 grid gap-4">{scrims.slice(0, 3).map((s) => <div key={s.id} className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/30 p-4"><div><b>{s.game}</b><p className="text-sm text-white/50">{s.organizer} • {s.startTime}</p></div><span className="text-arena-cyan">{s.slots.filled}/{s.slots.total}</span></div>)}</div><div className="mt-8 h-32 rounded-3xl border border-arena-cyan/20 bg-grid bg-[size:22px_22px] p-4"><div className="h-full rounded-2xl bg-gradient-to-r from-arena-purple/50 via-arena-cyan/25 to-transparent" /></div></div></Card>
        </motion.div>
      </section>
      ...
      <Section title="Loved by grinders" subtitle="Designed for competitive communities that need reliability."><div className="grid gap-5 md:grid-cols-3">{["Found scrims without chasing ten Discord links.", "Our paid lobbies finally look professional.", "Trust scores made new organizers safer to try."].map((quote, i) => <Card key={quote} className="p-6"><MessageCircle className="text-arena-purple" /><p className="mt-5 text-white/75">“{quote}”</p><p className="mt-4 text-sm text-white/45">Verified player #{i + 1}</p></Card>)}</div></Section>
    </main>
  );
}

function Section({ title, subtitle, children }: { title: string; subtitle: string; children: React.ReactNode }) {
  return <section className="container-arena py-14"><div className="mb-8 flex items-end justify-between gap-5"><div><h2 className="font-display text-3xl font-black sm:text-5xl">{title}</h2><p className="mt-3 text-white/55">{subtitle}</p></div><ShieldCheck className="hidden text-arena-cyan sm:block" /></div>{children}</section>;
}
